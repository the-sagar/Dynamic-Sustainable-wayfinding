...
public class FGS extends Service {
        ...
        if (e_fgs_ctrl_request != null) {
            switch (e_fgs_ctrl_request) {
                case FGS_COMMAND_START: {
                    ...
                    Log.i(TAG, "onStartCommand: Received START request " + s_class_harness);
                    ...
                    if (!m_is_running) {
                        ...
                        try {
                            ...
                            Log.d(TAG, "onStartCommand: mapped class " + s_class_harness);
                    ...
                    Log.d(TAG, "onStartCommand: start sequence complete - notifying...");
    ...
    private void deleteNotificationChannel() {
        ...
        nm.deleteNotificationChannel(Constants.ASNB_NOTIFICATIONS.NOTIFICATION_CHANNEL_ID__CONTROLLER_SERVICE);
    }
    ...
    public void onDestroy() {
        try {
            Log.i(TAG, "onDestroy: FGS is being destroyed (stopService) - notifying...");
...
//            }
            ...
            m_is_running = false;
            Log.d(TAG, "onDestroy: controller stopped - notifying...");
...
//            chan.setLightColor(Color.BLUE);
    ...
    private Notification fgs_asn__prepare(final String s_title, final String s_status) {
        ...
        Intent exitIntent = new Intent(this, FGS.class);
        exitIntent.setAction(Constants.Enums.E_INTENT_ACTION__REQUEST.FGS_COMMAND_STOP.toString());
        PendingIntent exitPendingIntent = PendingIntent.getService(this, 0, exitIntent, PendingIntent.FLAG_CANCEL_CURRENT);

        NotificationCompat.Action exitAction = new NotificationCompat.Action.Builder(R.drawable.ic_clear_white,
...
//            .setLargeIcon(BitmapFactory.decodeResource(getResources(), R.mipmap.ic_launcher))
    ...
    private void init() {
        ...
        try {
            File f_gpkg_root_dir = GPKG.F_GPKG_BUNDLE_ROOT_DIR.getInstance(getApplicationContext());
    ...
    private void get__tegola_version() {
        try {
            final File f_tegola_bin_executable = TEGOLA_BIN.getInstance(getApplicationContext()).getF_tegola_bin();
            final String
            ...
            String[] envp = {"LD_LIBRARY_PATH=" + SysUtil.Companion.LD_LIBRARY_PATH(getApplicationContext())};
            Process process_tegola_version = Runtime.getRuntime().exec(s_cmdline, envp);
            if (process_tegola_version != null) {
                ...
                if (m_s_version == null)
                    m_s_version = "unknown";
                TEGOLA_BIN.getInstance(getApplicationContext()).setF_tegola_version(m_s_version);
            }
        } catch (IOException e) {
            e.printStackTrace();
            try {
                TEGOLA_BIN.getInstance(getApplicationContext()).setF_tegola_version("unknown");
    ...
    private void handle_mvt_server_control_request__start(@NonNull final MVT_SERVER_START_SPEC server_start_spec) {
        try {
            TEGOLA_BIN.getInstance(getApplicationContext()).getF_tegola_bin();
    ...
    private boolean shell__tegola__start(@NonNull final MVT_SERVER_START_SPEC server_start_spec) throws IOException, Exceptions.UnsupportedCPUABIException, Exceptions.InvalidTegolaArgumentException, UnknownMVTServerStartSpecType, PackageManager.NameNotFoundException, Exceptions.TegolaBinaryNotExecutableException {
        ...
        final File
                ...
                , f_tegola_bin_executable = TEGOLA_BIN.getInstance(getApplicationContext()).getF_tegola_bin();
        final String
                s_tegola_bin_executable_path = f_tegola_bin_executable.getCanonicalPath()
                ;
        if (!f_tegola_bin_executable.exists())
        ...
        pb_env.put("LD_LIBRARY_PATH", SysUtil.Companion.LD_LIBRARY_PATH(getApplicationContext()));
        ...
        if (server_start_spec instanceof MVT_SERVER_START_SPEC__GPKG_PROVIDER) {
            ...
            try {
                f_gpkg_bundles_root_dir = GPKG.F_GPKG_BUNDLE_ROOT_DIR.getInstance(getApplicationContext());
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
...
//            Log.d(TAG, "shell__tegola__start: \tset pb env " + Constants.Strings.TEGOLA_PROCESS.TILE_CACHE.FILE.BASE_PATH_ENV_VAR + " to \"" + pb_env.get(Constants.Strings.TEGOLA_PROCESS.TILE_CACHE.FILE.BASE_PATH_ENV_VAR) + "\"");
        ...
        m_thread_tegola_process_stderr_monitor = new Thread(() -> {
...
//                            e.printStackTrace();
        ...
        m_thread_tegola_process_stdout_monitor = new Thread(() -> {
            boolean found_listen_port = false;
            String s_key_listen_port = "starting tegola server on port";
...
//                            e.printStackTrace();
        ...
        m_thread_tegola_process_monitor = new Thread(() -> {
            try {
